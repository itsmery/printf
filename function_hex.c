...
/**
 * function_hex  - Handles the lowercase '%x' hex conversion specifier
...
{
	unsigned long int numl;
	unsigned short int numh;
	unsigned int num;

	if (length == 'l')
	{
		numl = va_arg(args, unsigned long int);
		return (print_hex_l(numl, 0, flag));
	}
	else if (length == 'h')
	{
...
int handle_hex_upper(va_list args, char flag, char length)
{
	...
	{
		numl = va_arg(args, unsigned long int);
		return (print_hex_l(numl, 1, flag));
	}
	else if (length == 'h')
	{
		num_h = (unsigned short int)va_arg(args, unsigned int);
